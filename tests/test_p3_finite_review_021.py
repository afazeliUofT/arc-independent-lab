...
spec = importlib.util.spec_from_file_location("p3_finite_review_021_tested", LAB / "scripts/p3_finite_review_021.py")
finite = importlib.util.module_from_spec(spec)
spec.loader.exec_module(finite)


class FiniteReview021Tests(unittest.TestCase):
    def setUp(self):
    ...
    def execute(self, stage_callback, recheck=None):
        ...
        with mock.patch.object(finite, "run_stage", side_effect=stage_callback) as stages, \
             ...
             mock.patch.object(finite, "prior_failed_attempt", return_value={"synthetic_fixture_only": True}):
    ...
    def test_bundle_rejects_changed_source_and_frozen_input(self):
        ...
        with mock.patch.object(finite, "SCOPE_SHA256", finite.sha(scope_path.read_bytes())), \
             ...
             mock.patch.object(finite, "REQUIRED_CORRECTION_PINS", {"scripts/fixture_source.py"}), \
    ...
    def test_default_inspection_has_no_host_or_native_action(self):
        with mock.patch.object(finite, "load_bundle", return_value=(self.scope, {}, self.pins)), \
             ...
             mock.patch.object(finite.sys, "argv", ["p3_finite_review_021.py"]), \
    ...
    def test_complete_receipt_reused_before_host_or_approval_checks(self):
        ...
        with mock.patch.object(finite, "load_bundle", return_value=(self.scope, {}, self.pins)), \
             ...
             mock.patch.object(finite, "prior_failed_attempt", side_effect=AssertionError("prior rechecked")), \
             mock.patch.object(finite.sys, "argv", ["p3_finite_review_021.py", "--run-attended-review"]), \
             contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(finite.main(), 0)
    ...
    def test_partial_attempt_preserved_and_never_retried(self):
        ...
        with mock.patch.object(finite, "prior_failed_attempt", return_value={"synthetic_fixture_only": True}), \
    ...
    def stage_fixtures(self):
        ...
        modules = {
            ...
            "p3_reviewer_session_021": types.SimpleNamespace(run_session=session)}
    ...
    def prior_fixture(self):
        """Real historical receipt validators with synthetic bytes; no client."""
        modules = []
        for file_name in ("p3_finite_review", "p3_finite_review_020"):
            spec = importlib.util.spec_from_file_location(
                file_name + "_021_fixture", LAB / "scripts" / (file_name + ".py"))
            value = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(value)
            modules.append(value)
        original, correction = modules
        stack = contextlib.ExitStack()
        self.addCleanup(stack.close)
        for value in modules:
            stack.enter_context(mock.patch.object(value, "ROOT", self.root))
            stack.enter_context(mock.patch.object(value, "SCOPE_SHA256", finite.SCOPE_SHA256))
            stack.enter_context(mock.patch.object(value, "load_bundle",
                                                  return_value=(self.scope, {}, self.pins)))
        stack.enter_context(mock.patch.object(correction, "module", return_value=original))
        stack.enter_context(mock.patch.object(finite, "module", return_value=correction))
        runs, reports = [], []
        for value in modules:
            run = self.root / "delivery" / value.RUN_NAME
            run.mkdir()
            observation = {"synthetic_fixture_only": True, "client_started": True,
                           "native_process_reaped": True, "thread_request_sent": False,
                           "model_turn_request_sent": False, "model_turn_request_queued": False,
                           "verdict_submitted": False}
            session_raw = (json.dumps(observation) + "\n").encode()
            (run / "synthetic_SESSION.json").write_bytes(session_raw)
            report = {"kind": value.REPORT_KIND, "pins": self.pins,
                      "status": "STOPPED_WITHOUT_COMPLETED_REVIEW",
                      "unattended_model_use_authorized": False,
                      "parent_credential_contents_read": False, "reviewer_output": None,
                      "preserved_session_receipts": [{"path": "synthetic_SESSION.json",
                                                      "sha256": finite.sha(session_raw)}],
                      "stages": [{"stage": "synthetic", "observation": observation}]}
            if value is correction:
                report["prior_attempt"] = correction.prior_failed_attempt()
            raw = finite.exclusive_json(run / "REPORT.json", report)
            (run / "REPORT.sha256").write_text(finite.sha(raw) + "\n")
            if value is original:
                stack.enter_context(mock.patch.object(correction, "PRIOR_REPORT_SHA256", finite.sha(raw)))
            else:
                stack.enter_context(mock.patch.object(finite, "PRIOR_REPORT_SHA256", finite.sha(raw)))
            runs.append(run)
            reports.append(report)
        return original, correction, runs, reports

    def test_prior_receipts_guard_accepts_exact_two_unused_attempts_without_changes(self):
        _, _, runs, _ = self.prior_fixture()
        before = [{p.name: p.read_bytes() for p in run.iterdir()} for run in runs]
        result = finite.prior_failed_attempt()
        self.assertTrue(result["unchanged_correction020_receipt_verified"])
        self.assertTrue(result["original_attempt"]["unchanged_original_receipt_verified"])
        self.assertEqual(result["report_sha256"], finite.sha((runs[1] / "REPORT.json").read_bytes()))
        self.assertEqual(result["prior_explicit_model_turns"], 0)
        self.assertEqual(result["prior_failed_native_processes"], 2)
        self.assertEqual(result["corrective_invocation_native_processes_maximum"], 2)
        self.assertEqual(result["cumulative_native_processes_maximum_including_failed_attempts"], 4)
        self.assertEqual(result["cumulative_explicit_model_turns_maximum"], 2)
        self.assertEqual([{p.name: p.read_bytes() for p in run.iterdir()} for run in runs], before)
        self.assertFalse(self.run.exists())

    def test_missing_either_prior_receipt_stops_before_creating_correction_directory(self):
        _, _, runs, _ = self.prior_fixture()
        for run in runs:
            for name in ("REPORT.json", "REPORT.sha256", "synthetic_SESSION.json"):
                with self.subTest(run=run.name, name=name):
                    source = run / name
                    parked = run / (name + ".preserved")
                    source.rename(parked)
                    try:
                        with mock.patch.object(finite, "run_stage") as stage, \
                             self.assertRaises((finite.Stop, OSError)):
                            finite.execute(self.scope, {}, self.pins, {}, self.packet_info)
    ...
    def test_changed_either_prior_report_rejected_even_with_updated_checksum(self):
        _, _, runs, _ = self.prior_fixture()
        for run in runs:
            with self.subTest(run=run.name):
                raw = (run / "REPORT.json").read_bytes()
                changed = raw + b" "
                (run / "REPORT.json").write_bytes(changed)
                (run / "REPORT.sha256").write_text(finite.sha(changed) + "\n")
                with self.assertRaises(finite.Stop):
                    finite.prior_failed_attempt()
                self.assertEqual((run / "REPORT.json").read_bytes(), changed)
                self.assertFalse(self.run.exists())
                (run / "REPORT.json").write_bytes(raw)
                (run / "REPORT.sha256").write_text(finite.sha(raw) + "\n")

    def test_prior020_model_thread_cleanup_or_science_stage_prevents_correction(self):
        _, _, runs, reports = self.prior_fixture()
        run, base_report = runs[1], reports[1]
        changes = ("sent", "queued", "second_stage", "thread", "not_reaped", "not_started",
                   "verdict", "prior_count", "prior_scope", "prior_missing")
        for case in changes:
            with self.subTest(case=case):
                report = json.loads(json.dumps(base_report))
                observation = report["stages"][0]["observation"]
                if case == "second_stage":
                    report["stages"].append({"stage": "science", "observation": {}})
                elif case == "thread":
                    observation["thread_request_sent"] = True
                elif case == "not_reaped":
                    observation["native_process_reaped"] = False
                elif case == "not_started":
                    observation["client_started"] = False
                elif case == "verdict":
                    observation["verdict_submitted"] = True
                elif case == "prior_count":
                    report["prior_attempt"]["prior_explicit_model_turns"] = 1
                elif case == "prior_scope":
                    report["prior_attempt"]["access_scope_sha256"] = "c" * 64
                elif case == "prior_missing":
                    del report["prior_attempt"]
                else:
                    observation["model_turn_request_" + case] = True
                raw = (json.dumps(report) + "\n").encode()
                (run / "REPORT.json").write_bytes(raw)
                (run / "REPORT.sha256").write_text(finite.sha(raw) + "\n")
                # Re-pin only synthetic fixtures to exercise semantic guards.
                with mock.patch.object(finite, "PRIOR_REPORT_SHA256", finite.sha(raw)), \
                     mock.patch.object(finite, "run_stage") as stage, \
                     self.assertRaises(finite.Stop):
                    finite.execute(self.scope, {}, self.pins, {}, self.packet_info)
                stage.assert_not_called()
                self.assertFalse(self.run.exists())

    def test_original019_model_turn_cannot_be_lost_from_chain(self):
        _, correction, runs, reports = self.prior_fixture()
        for field in ("model_turn_request_sent", "model_turn_request_queued"):
            report = json.loads(json.dumps(reports[0]))
            report["stages"][0]["observation"][field] = True
            raw = (json.dumps(report) + "\n").encode()
            (runs[0] / "REPORT.json").write_bytes(raw)
            (runs[0] / "REPORT.sha256").write_text(finite.sha(raw) + "\n")
            with mock.patch.object(correction, "PRIOR_REPORT_SHA256", finite.sha(raw)), \
                 mock.patch.object(finite, "run_stage") as stage, \
                 self.assertRaises(finite.Stop):
                finite.execute(self.scope, {}, self.pins, {}, self.packet_info)
            stage.assert_not_called()
    ...
    def test_changed_or_unrecorded_either_prior_session_stops_correction(self):
        _, _, runs, _ = self.prior_fixture()
        for run in runs:
            with self.subTest(run=run.name):
                path = run / "synthetic_SESSION.json"
                original = path.read_bytes()
                path.write_bytes(b"changed synthetic session bytes\n")
                with self.assertRaises(finite.Stop):
                    finite.prior_failed_attempt()
                path.write_bytes(original)
                extra = run / "science_SESSION.json"
                extra.write_bytes(b"unrecorded synthetic science session\n")
                with self.assertRaises(finite.Stop):
                    finite.prior_failed_attempt()
    ...
    def test_missing_integration_dependency_pin_fails_closed(self):
        path = self.root / finite.SCOPE_PATH
        path.write_text("{}")
        with mock.patch.object(finite, "SCOPE_SHA256", finite.sha(path.read_bytes())), \
             mock.patch.object(finite, "SOURCE_PINS", {"scripts/fixture.py": "a" * 64}):
            with self.assertRaisesRegex(finite.Stop, "Unfinished or changed review scope/pins"):
                finite.load_bundle()
