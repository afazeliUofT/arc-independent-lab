...
SPEC = importlib.util.spec_from_file_location("collector032", HERE.parent / "scripts" / "publish_review032_evidence.py")
...
class EvidenceCollectionTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="collector032_", dir=HERE)
    ...
    def test_partial_attempt_has_no_fabricated_main_report(self):
        raw = b'{ "status": "FAILED", "no_native_start": true }\r\n'
        self.write(c.MAIN + "ATTEMPT.json", raw)
        result, archive, index = self.run_collection()
        self.assertEqual(result["status_summary"]["main_report"], "MISSING")
        self.assertEqual((archive / "files" / (c.MAIN + "ATTEMPT.json")).read_bytes(), raw)
    ...
    def test_fixed_workflow_files_allow_missing032_to_be_explained(self):
        for name in ("WORKFLOW_REPORT.json", "LAUNCH_RESERVED.json", "LAUNCH_OUTCOME.json"):
            self.write(c.WORKFLOW + name)
        result, archive, index = self.run_collection()
        self.assertEqual(result["status_summary"]["collection_status"], "WORKFLOW_EVIDENCE_ONLY_NO_032_RECEIPTS")
        self.assertEqual(result["status_summary"]["original032_file_count"], 0)
