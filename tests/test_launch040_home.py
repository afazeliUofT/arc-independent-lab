...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/launch040_home.py'
SPEC = importlib.util.spec_from_file_location('launch040', SCRIPT)
...
class LauncherTests(unittest.TestCase):
    def setUp(self):
        ...
        self.members.update({
            ...
            **{'scripts/' + name: b'# Fixture script; never executed.\n' for name in
               ('review040_workflow.py', 'stage_source_packet038.py',
                'review032_workflow.py', 'publish_review040_evidence.py')}
        })
    ...
    def test_valid_package_copies_exact_bytes_is_private_and_repeatable(self):
        ...
        self.assertEqual(command[3:], [str(bundle / 'scripts/review040_workflow.py'),
    ...
    def test_matching_partial_extraction_is_completed(self):
        ...
        path = destination / 'bundle' / 'README_040.md'
        path.parent.mkdir(parents=True)
        path.write_bytes(self.members['README_040.md'])
    ...
    def test_public_returned_report_sha256_is_inventory_checked(self):
        name = ('public/artifacts/P3_REVIEW_039_RETURN/fixture/files/'
                'delivery/P3_FOCUSED_REVIEW_039/REPORT.sha256')
        digest = b'5' * 64 + b'\n'
        raw = self.archive({**self.members, name: digest})
        self.assertEqual(launcher.archive_payloads(raw)[name], digest)
        bundle = self.run_launch(raw)
        self.assertEqual((bundle / name).read_bytes(), digest)
        damaged = self.archive({**self.members, name: digest}, overrides={name: b'6' * 64 + b'\n'})
        with self.assertRaisesRegex(launcher.LaunchStop, 'digest mismatch'):
            launcher.archive_payloads(damaged)

    def test_sha256_member_allowance_is_public_only(self):
        for name in ('scripts/REPORT.sha256', 'packet/REPORT.sha256',
                     'REPORT.sha256', 'public/REPORT.bin'):
            with self.subTest(name=name):
                with self.assertRaisesRegex(launcher.LaunchStop, 'Unexpected package member'):
                    launcher.archive_payloads(self.archive({**self.members, name: b'0' * 64 + b'\n'}))
    ...
    def test_staged_evidence_recovery_is_delegated_without_altering_index(self):
        path = self.lab / 'artifacts/P3_REVIEW_040_RETURN/fixture/INDEX.json'
        path.parent.mkdir(parents=True)
        ...
        self.assertEqual(self.calls[0][0][3], str(bundle / 'scripts/review040_workflow.py'))
    ...
    def test_failed_commit_recovery_through_real_launcher_and_workflow(self):
        ...
        spec = importlib.util.spec_from_file_location('home_workflow040_fixture', root / 'scripts/review040_workflow.py')
        ...
        self.members['RELEASE.json'] = workflow.safe.canonical({
            'kind': 'P3_FOCUSED_REVIEW_040_RELEASE_v1', 'repository': workflow.REPOSITORY,
        ...
        def execute(command, **kwargs):
            # Import the exact launcher-selected, extracted workflow and its
            # extracted siblings, so a version-wiring error is observable here.
            spec = importlib.util.spec_from_file_location('extracted040_workflow', command[3])
            extracted = importlib.util.module_from_spec(spec)
            # The actual launcher subprocess carries -B, so model that flag in
            # this in-process substitute rather than creating extra cache files.
            self.assertIn('-B', command[1:3])
            with patch.object(sys, 'dont_write_bytecode', True):
                spec.loader.exec_module(extracted)
            self.assertEqual(extracted.PACKET, 'delivery/P3_FOCUSED_REVIEW_PACKET_040')
            self.assertEqual(extracted.publisher.ARCHIVE_PARENT, 'artifacts/P3_REVIEW_040_RETURN')
            try:
                extracted.workflow(self.lab, Path(command[-1]), git_runner=git_runner,
                                   controller_runner=controller, emit=lambda value: None)
                return subprocess.CompletedProcess(command, 0)
            except extracted.safe.StageStop:
                return subprocess.CompletedProcess(command, 1)
        raw = self.archive()
        with self.assertRaisesRegex(launcher.LaunchStop, 'Review workflow stopped'):
    ...
    def test_minimal_initial_sibling_dependency_closure_imports_offline(self):
        ...
        minimal.mkdir()
        names = ('review040_workflow.py', 'stage_source_packet038.py',
                 'review032_workflow.py', 'publish_review040_evidence.py')
        for name in names:
        ...
        code = ("import importlib.util; s=importlib.util.spec_from_file_location('import_fixture', "
                + repr(str(minimal / 'review040_workflow.py'))
        ...
        self.assertEqual(result.stdout.decode().strip(), 'delivery/P3_FOCUSED_REVIEW_PACKET_040')
...
SPEC = importlib.util.spec_from_file_location('bootstrap040',
    Path(__file__).resolve().parents[1] / 'scripts/downloads_bootstrap040.py')
...
class BootstrapTests(unittest.TestCase):
    def setUp(self):
        ...
        with zipfile.ZipFile(out, 'w') as archive:
            archive.writestr('launch040_home.py', code)
    ...
    def test_bootstrap_is_one_literal_python_here_document(self):
        source = (Path(__file__).resolve().parents[1] / 'scripts/downloads_bootstrap040.py').read_text()
        filled = source.replace('__ZIP_SHA256__', self.sha)
        compile(filled, 'download040_fixture.py', 'exec')
        command = "python3 - <<'PY'\n" + filled.rstrip() + "\nPY\n"
