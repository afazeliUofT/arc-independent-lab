...
SPEC = importlib.util.spec_from_file_location('workflow040_fixture', ROOT / 'scripts/review040_workflow.py')
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_one_invocation_and_idempotent_receipt_only_retry(self):
        ...
        self.assertTrue(all(name.startswith('artifacts/P3_REVIEW_040_RETURN/') for name in names))
    ...
    def test_invocation_uses_only_fresh040_controller_and_packet(self):
        with patch.object(w.transport, 'run_controller', return_value={'fixture': True}) as run:
            result = w.invoke(self.lab, emit=lambda value: None)
        self.assertEqual(result, {'fixture': True})
        command = run.call_args.kwargs['command']
        self.assertEqual(command[1:], ['-I', '-B', '-u',
            str(self.lab / 'scripts/p3_focused_review_040.py'), '--run-attended-review',
            '--packet', str(self.lab / 'delivery/P3_FOCUSED_REVIEW_PACKET_040')])
        self.assertEqual(run.call_args.kwargs['wall_seconds'], 4310)

    def test_prior039_reservation_is_preserved_and_does_not_reuse039(self):
        path = self.lab / 'delivery/P3_039_RETURN_WORKFLOW/LAUNCH_RESERVED.json'
        path.parent.mkdir(parents=True)
        raw = b'{"kind":"prior039_fixture","already_used":true}\n'
        path.write_bytes(raw)
        result = self.run_workflow()
        self.assertEqual(path.read_bytes(), raw)
        self.assertEqual(self.native_calls, 1)
        self.assertFalse(result['collect_only'])
        self.assertTrue((self.lab / w.WORKFLOW / 'LAUNCH_RESERVED.json').is_file())
    ...
    def test_failed_commit_retries_only_exact_staged_archive(self):
        ...
        result = self.run_workflow()
        self.assertEqual(self.native_calls, 1)
        self.assertEqual(result['status'], '040_EVIDENCE_PUBLISHED')
    ...
    def test_active_workflow_lock_stops_concurrent_launch(self):
        ...
        try:
            ...
            with self.assertRaisesRegex(w.safe.StageStop, '040_workflow_already_active_no_second_launch'):
                self.run_workflow()
        finally:
            os.close(descriptor)
    ...
    def test_prior_reservation_without_controller_files_returns_unknown_usage(self):
        ...
        self.assertEqual(summary['collection_status'], 'WORKFLOW_EVIDENCE_ONLY_NO_040_RECEIPTS')
    ...
    def test_unrelated_commit_stops_and_never_pushes_or_starts(self):
        ...
        head = self.git('rev-parse', 'HEAD')
        with self.assertRaisesRegex(w.safe.StageStop, 'unpublished_commit_is_not_exact040_evidence'):
            self.run_workflow()
        self.assertEqual(self.native_calls, 0)
        self.assertEqual(self.git('rev-parse', 'HEAD'), head)
    ...
    def test_unignored_private_destination_stops_before_any_packet_copy(self):
        ...
        with self.assertRaisesRegex(w.safe.StageStop, '040_private_runtime_paths_not_ignored'):
            self.run_workflow()
        self.assertEqual(self.native_calls, 0)
    ...
    def test_wrong_private_packet_stops_and_returns_categorical_evidence(self):
        ...
        result = self.run_workflow()
        self.assertEqual(self.native_calls, 0)
        self.assertEqual(result['collection_summary']['collection_status'], 'WORKFLOW_EVIDENCE_ONLY_NO_040_RECEIPTS')
        self.assertFalse((self.lab / w.PACKET).exists())
        report = (self.lab / w.WORKFLOW / 'WORKFLOW_REPORT.json').read_text()
        self.assertIn('040_private_packet_hash_mismatch', report)
