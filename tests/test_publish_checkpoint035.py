...
spec = importlib.util.spec_from_file_location('publication035_under_test', HERE.parent / 'scripts' / 'publish_checkpoint035.py')
...
class PublisherTests(unittest.TestCase):
    def setUp(self):
        self.temporary = tempfile.TemporaryDirectory(prefix='publisher035-fixture-', dir=HERE)
    ...
    def manifest(self, name='scripts/fixed035.py', old=None, new=b'new\n'):
    ...
    def pending(self, name='artifacts/P3_REVIEW_035_RETURN/fixed/REPORT.json', body=b'{"status":"no_verdict"}\n'):
        self.put(name, body)
        value = {'kind': 'P2_035_PENDING_PUBLICATION_v1', 'commit_message':pub.COMMIT_MESSAGE, 'parent': self.fake.parent, 'package_manifest_sha256':'0'*64,
                 'files': {name: {'sha256': digest(body)}},
                 'return_index': 'artifacts/P3_REVIEW_035_RETURN/fixed/INDEX.json',
    ...
    def test_install_refuses_unrelated_staged_and_preserves_bytes(self):
        ...
        self.assertEqual((self.root / 'scripts/fixed035.py').read_bytes(), b'old')
    ...
    def test_install_refuses_unknown_declared_working_version(self):
        manifest = self.manifest(old=b'old')
        self.put('scripts/fixed035.py', b'human edit')
        with self.assertRaises(RuntimeError):
            pub.install(self.root, self.bundle, manifest)
        self.assertEqual((self.root / 'scripts/fixed035.py').read_bytes(), b'human edit')

    def test_install_refuses_unknown_declared_staged_version(self):
        manifest = self.manifest(old=b'old')
        self.fake.index['scripts/fixed035.py'] = b'human staged edit'
        with self.assertRaises(RuntimeError):
            pub.install(self.root, self.bundle, manifest)
        self.assertEqual(self.fake.index['scripts/fixed035.py'], b'human staged edit')
        self.assertEqual((self.root / 'scripts/fixed035.py').read_bytes(), b'old')
    ...
    def test_install_accepts_exact_declared_transition_and_repeat(self):
        ...
        pub.install(self.root, self.bundle, manifest)
        pub.install(self.root, self.bundle, manifest)
        self.assertEqual((self.root / 'scripts/fixed035.py').read_bytes(), b'new\n')
        self.assertEqual(self.fake.commit_count, 0)

    def test_exact_staging_preserves_crlf_bytes(self):
        raw = b'key,value\r\na,b\r\n'
        path = 'artifacts/P3_REVIEW_035_RETURN/fixed/metrics.csv'
        self.put(path, raw)
    ...
    def package(self):
        ...
        value.update(kind='P2_CHECKPOINT_035_PACKAGE_v1', publication_only=True, execution_started=False)
    ...
    def test_drive_publishes_once_and_repeat_does_not_commit_or_push(self):
        ...
        self.assertEqual(set(self.fake.trees[self.fake.head]), {'scripts/fixed035.py'})
    ...
    def test_changed_package_stops_pending_retry_before_push(self):
        ...
        with mock.patch.object(pub,'repository_check'), mock.patch('builtins.print'):
            self.assertEqual(pub.drive(self.root,self.bundle),128)
            manifest['files']['scripts/fixed035.py']['old_sha256']='a'*64
