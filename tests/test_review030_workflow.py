...
spec = importlib.util.spec_from_file_location('workflow031_tested', HERE.parent / 'scripts' / 'review030_workflow.py')
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_each_existing_partial_path_prevents_controller_launch(self):
        for relative in (w.MAIN, w.KERNEL, w.WORKFLOW + '/LAUNCH_RESERVED.json'):
    ...
    def test_changed_published_answer_cannot_launch_and_creates_report(self):
        ...
        self.assertEqual(self.report()['reason_code'], 'approved030_pinned_input_mismatch')
        self.assertEqual(self.calls, [])
    ...
    def test_attempt_appearing_during_pin_verification_prevents_launch(self):
        ...
        def race(root, relative, maximum=4 * 1024 * 1024):
            ...
            (root / w.KERNEL).mkdir(parents=True, exist_ok=True)
            return raw
        w.read_fixed = race
        try:
            result = self.drive()
