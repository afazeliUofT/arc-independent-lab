...
import p3_reviewer_session_028 as session
import test_p3_deprecation_correction_024 as fixtures
import test_p3_warning_session_026 as transport_fixture

CANARY = 'UNKNOWN_WARNING_PRIVATE_028_89cf71'
REPORTS = []
BINDING = {'model': 'gpt-5.6-sol', 'id': 'gpt-5.6-sol', 'effort': 'max'}


# Fixed offline responder acknowledges each real parent tool response before
# advancing. No native client, model, credential or network is used.
CHILD = transport_fixture.CHILD.replace(
    'emit_group(fixture.get("after_read", []))',
    'emit_group(fixture.get("after_response", {}).get(str(request["id"]), fixture.get("after_read", [])))')
...
class SessionTests(unittest.TestCase):
    ...
    def run_fixture(self, mode='synthetic', expected_binding=None, *, verdict_expected=False):
        source = self.root / 'offline_responder_028.py'
        source.write_text(CHILD)
        fixture = self.root / 'fixture_028.json'
        ...
        with session.broker_module.ReviewBroker(self.packet,
            ...
            report = session.run_session({'argv': [sys.executable, '-I', '-B', str(source), str(fixture)]},
                ...
                wall_seconds=10, max_calls=12, expected_binding=expected_binding)
    ...
    def test_oversized_canary_read_recovers_then_real_path_refusal(self):
        first = fixtures.tool_call(101, {**session.SYNTHETIC_READ, 'length': 200000})
        self.fixture['after']['turn/start'] = [first]
        self.fixture['after_read'] = []
        self.fixture['after_response'] = {
            '101': [fixtures.tool_call(102, session.SYNTHETIC_READ)],
            '102': [fixtures.tool_call(103, session.SYNTHETIC_FORBIDDEN)]}
        report = self.run_fixture()
        ...
        self.assertEqual(report['server_requests_dispatched'], 2)
        self.assertEqual(report['broker_receipts'][0]['error_code'], 'integer_bounds')
        self.assertFalse(report['broker_receipts'][0]['successful'])
        self.assertTrue(report['broker_receipts'][1]['successful'])
        self.assertTrue(report['observed_refusal'])
        self.assertEqual(report['last_server_request_observation']['method'], 'item/tool/call')
        self.assertTrue(report['last_server_request_observation']['request_envelope_fields_exact'])
        self.assertNotEqual(report['last_notification_type'], 'unknown')
        self.assertIsNotNone(report['broker_boundary_failure'])

    def test_science_arguments_can_be_corrected_without_successful_call(self):
        self.fixture['after']['turn/start'] = [fixtures.tool_call(101,
            {**session.SYNTHETIC_READ, 'length': 200000})]
        self.fixture['after_read'] = []
        self.fixture['after_response'] = {'101': [{
            'method': 'turn/completed', 'params': {'threadId': 'thread-test',
                'turn': {'id': 'turn-test', 'items': [], 'status': 'completed'}}}]}
        report = self.run_fixture(mode='science', expected_binding=BINDING)
        self.assertEqual(report['status'], 'STOPPED_WITHOUT_VERDICT')
        self.assertEqual(report['recoverable_input_errors'], 1)
        self.assertEqual(len(report['recoverable_input_error_receipts']), 1)
        self.assertFalse(any(row['successful'] for row in report['broker_receipts']))
        self.assertEqual(report['last_server_request_observation']['tool'], 'read_text')
        self.assertIsNone(report['broker_boundary_failure'])

    def test_ninth_argument_error_is_terminal_and_does_not_write_verdict(self):
        def invalid(number):
            return fixtures.tool_call(number, {**session.SYNTHETIC_READ, 'length': 200000})
        self.fixture['after']['turn/start'] = [invalid(101)]
        self.fixture['after_read'] = []
        self.fixture['after_response'] = {str(i): [invalid(i + 1)] for i in range(101, 109)}
        report = self.run_fixture(mode='science', expected_binding=BINDING)
        self.assertEqual(report['status'], 'STOPPED_WITHOUT_VERDICT')
        self.assertEqual(report['recoverable_input_errors'], 8)
        self.assertEqual(report['failed_broker_calls'], 9)
        self.assertEqual(len(report['broker_receipts']), 8)
        self.assertTrue(all(not row['successful'] for row in report['broker_receipts']))
        self.assertEqual(report['broker_boundary_failure']['layer'], 'dispatch')

    def test_malformed_verdict_repaired_then_exclusive_dependency_fixture(self):
        manifest = json.loads((self.packet / 'BROKER_MANIFEST.json').read_bytes())
        hashes = {}
        for name, path in session.broker_module.REVIEW_MANIFESTS.items():
            data = b'{"scope":"SYNTHETIC ENGINEERING FIXTURE ONLY"}\n'
        ...
        verdict = {'verdict': 'SUSPEND_FOR_DEPENDENCY',
            ...
            'strongest_objections': [], 'missing_dependencies': ['Synthetic fixture only'],
            'required_corrections': []}
        wrong = fixtures.tool_call(101, {**verdict, 'summary': ''})
        corrected = fixtures.tool_call(102, verdict)
        wrong['params']['tool'] = corrected['params']['tool'] = 'submit_verdict'
        self.fixture['after']['turn/start'] = [wrong]
        self.fixture['after_read'] = []
        self.fixture['after_response'] = {'101': [corrected]}
        report = self.run_fixture(mode='science', expected_binding=BINDING, verdict_expected=True)
        self.assertEqual(report['status'], 'VERDICT_SUBMITTED')
        self.assertEqual(report['recoverable_input_errors'], 1)
        self.assertEqual(report['broker_receipts'][0]['error_code'], 'bounded_text')
        self.assertFalse(report['broker_receipts'][0]['successful'])
        self.assertTrue(report['broker_receipts'][1]['successful'])
        path = self.output / session.broker_module.VERDICT_NAME
        self.assertEqual(json.loads(path.read_bytes())['reviewer_verdict'], verdict)
    ...
    def test_foreign_bound_call_is_envelope_failure_without_dispatch(self):
        self.fixture['after']['turn/start'][0]['params']['threadId'] = CANARY
        report = self.run_fixture(mode='science', expected_binding=BINDING)
        self.assertEqual(report['status'], 'STOPPED_WITHOUT_VERDICT')
        self.assertEqual(report['broker_receipts'], [])
        self.assertEqual(report['broker_boundary_failure']['layer'], 'envelope')
        self.assertEqual(report['broker_boundary_failure']['code'], 'foreign_thread_or_turn')
        self.assertEqual(report['server_requests_dispatched'], 0)

    def test_unknown_dispatch_exception_text_is_not_retained(self):
        with mock.patch.object(session.broker_module.ReviewBroker, 'dispatch',
    ...
    def test_audit_deadline_applies_to_execution_but_not_repeated_call_feedback(self):
        with session.broker_module.ReviewBroker(self.packet,
                manifest_sha256=self.manifest_sha, output_root=self.output) as broker:
            witness = session._BrokerWitness(broker, 'science', time.monotonic() + 1)
            with mock.patch.object(broker, 'dispatch', wraps=broker.dispatch) as dispatch:
                with self.assertRaisesRegex(session.Stop, 'Insufficient remaining wall time'):
                    witness.dispatch('run_observer_audit', {})
                dispatch.assert_not_called()
            # Generated engineering state only: do not execute or claim an
            # actual auditor. This fixture isolates the already-run branch.
            broker._original._audited = True
            self.assertTrue(broker.audit_completed)
            boundary = session.protocol.DynamicToolBoundary(witness,
                thread_id='thread-test', turn_id='turn-test', max_calls=2, wall_seconds=5)
            call = fixtures.tool_call(101, {})
            call['params']['tool'] = 'run_observer_audit'
            response = json.loads(boundary.handle(json.dumps(call).encode()))
            self.assertFalse(response['result']['success'])
            self.assertEqual(json.loads(response['result']['contentItems'][0]['text'])['code'],
                             'audit_already_run')
            self.assertEqual(boundary.recoverable_input_errors, 1)
            self.assertFalse(boundary.stopped)
            self.assertFalse(boundary.finished)
            self.assertEqual(list(self.output.iterdir()), [])
