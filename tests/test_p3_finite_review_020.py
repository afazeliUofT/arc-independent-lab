...
spec = importlib.util.spec_from_file_location("p3_finite_review_020_tested", LAB / "scripts/p3_finite_review_020.py")
finite = importlib.util.module_from_spec(spec)
spec.loader.exec_module(finite)


class FiniteReview020Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_default_inspection_has_no_host_or_native_action(self):
        with mock.patch.object(finite, "load_bundle", return_value=(self.scope, {}, self.pins)), \
             ...
             mock.patch.object(finite.sys, "argv", ["p3_finite_review_020.py"]), \
    ...
    def test_complete_receipt_reused_before_host_or_approval_checks(self):
        ...
        with mock.patch.object(finite, "load_bundle", return_value=(self.scope, {}, self.pins)), \
             ...
             mock.patch.object(finite.sys, "argv", ["p3_finite_review_020.py", "--run-attended-review"]), \
             contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(finite.main(), 0)
    ...
    def stage_fixtures(self):
        ...
        modules = {
            ...
            "p3_reviewer_session_020": types.SimpleNamespace(run_session=session)}
    ...
    def prior_fixture(self):
        """Real original receipt validator, synthetic bytes, no native operation."""
        original_spec = importlib.util.spec_from_file_location(
            "p3_finite_review_019_guard_fixture", LAB / "scripts/p3_finite_review.py")
        original = importlib.util.module_from_spec(original_spec)
        original_spec.loader.exec_module(original)
        run = self.root / "delivery" / original.RUN_NAME
        run.mkdir()
        session_raw = b'{"synthetic_fixture_only":true,"model_turn_request_sent":false}\n'
        (run / "synthetic_SESSION.json").write_bytes(session_raw)
        report = {
            "kind": original.REPORT_KIND, "pins": self.pins,
            "unattended_model_use_authorized": False,
            "parent_credential_contents_read": False, "reviewer_output": None,
            "preserved_session_receipts": [{"path": "synthetic_SESSION.json",
                                            "sha256": finite.sha(session_raw)}],
            "stages": [{"observation": {"model_turn_request_sent": False,
                                         "model_turn_request_queued": False}}]}
        raw = finite.exclusive_json(run / "REPORT.json", report)
        (run / "REPORT.sha256").write_text(finite.sha(raw) + "\n")
        stack = contextlib.ExitStack()
        self.addCleanup(stack.close)
        stack.enter_context(mock.patch.object(original, "load_bundle",
                                              return_value=(self.scope, {}, self.pins)))
        stack.enter_context(mock.patch.object(finite, "module", return_value=original))
        stack.enter_context(mock.patch.object(finite, "PRIOR_REPORT_SHA256", finite.sha(raw)))
        return run, report, raw

    def test_prior_receipt_guard_accepts_exact_unused_attempt_without_changes(self):
        run, _, raw = self.prior_fixture()
        before = {p.name: p.read_bytes() for p in run.iterdir()}
        result = finite.prior_failed_attempt()
        self.assertTrue(result["unchanged_original_receipt_verified"])
        self.assertEqual(result["report_sha256"], finite.sha(raw))
        self.assertEqual(result["prior_explicit_model_turns"], 0)
        self.assertEqual({p.name: p.read_bytes() for p in run.iterdir()}, before)
        self.assertFalse(self.run.exists())

    def test_missing_prior_receipts_stop_before_creating_correction_directory(self):
        run, _, _ = self.prior_fixture()
    ...
    def test_changed_prior_report_rejected_even_with_matching_updated_checksum(self):
        run, _, raw = self.prior_fixture()
        changed = raw + b" "
    ...
    def test_prior_model_sent_queued_or_science_stage_prevents_correction(self):
        run, original_report, _ = self.prior_fixture()
        for case in ("sent", "queued", "second_stage"):
            with self.subTest(case=case):
                report = json.loads(json.dumps(original_report))
                if case == "second_stage":
                    report["stages"].append({"observation": {}})
                else:
                    report["stages"][0]["observation"]["model_turn_request_" + case] = True
                raw = (json.dumps(report) + "\n").encode()
                (run / "REPORT.json").write_bytes(raw)
                (run / "REPORT.sha256").write_text(finite.sha(raw) + "\n")
                # Re-pin only the synthetic fixture to exercise semantic guards.
    ...
    def test_changed_or_unrecorded_prior_session_stops_correction(self):
        run, _, _ = self.prior_fixture()
