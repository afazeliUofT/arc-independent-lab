...
mutation is implemented. The failed019 attempt and every original controller are preserved unchanged.
...
RUN_NAME = 'P3_FINITE_REVIEW_019_REPAIR_001'
REPORT_KIND = 'P3_FINITE_REVIEW_019_REPAIR_001_v1'
PRIOR_REPORT_SHA256 = '2725b63acd12a09a89041760ca9ea21f0f004a0ece1a6e4d2b7df125a48d54f6'
...
def run_stage(scope, prior_scope, run, mode, packet, manifest_sha, expected_binding=None):
    ...
    session = module('p3_reviewer_session_020')
...
def prior_failed_attempt():
    """Read only the exact preserved019 receipt; no native/log/auth operations."""
    original = module('p3_finite_review')
    try:
        _, _, original_pins = original.load_bundle()
        path = original.existing(ROOT / 'delivery' / original.RUN_NAME, original_pins)
        require(path is not None, 'The preserved original019 attempt is required')
        raw = bounded(path)
        require(sha(raw) == PRIOR_REPORT_SHA256,
                'Original019 report differs; preserve it and stop')
    except (original.Stop, OSError, ValueError, KeyError):
        raise Stop('Original019 receipt, pins or session files differ; preserve them') from None
    value = json.loads(raw)
    require(len(value['stages']) == 1 and value['reviewer_output'] is None and
            value['stages'][0]['observation']['model_turn_request_sent'] is False and
            value['stages'][0]['observation']['model_turn_request_queued'] is False,
            'Original019 operation used a model turn; no correction admitted')
    return {'report_path':str(path.relative_to(ROOT)), 'report_sha256':sha(raw),
            'unchanged_original_receipt_verified':True,
            'prior_explicit_model_turns':0, 'prior_failed_native_processes':1,
            ...
            'cumulative_native_processes_maximum_including_failed_attempt':3,
...
def execute(scope, prior_scope, pins, authorization, packet_info):
    ...
    result = {'kind':REPORT_KIND,'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'correction':'Exact startup advisory classification; all other warnings remain stops'}
    try:
...
def main():
    ...
    if not args.run_attended_review:
        ...
        print('Manual correction preserves the original019 attempt and uses a new report folder.')
