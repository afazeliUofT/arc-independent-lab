...
ZIP_NAME = 'ARC_Independent_Lab_038_HOME.zip'
...
def main():
    ...
    for folder in dict.fromkeys(folders):
        for archive in sorted(folder.glob(ZIP_NAME.removesuffix('.zip') + '*.zip')):
            try:
                if archive.is_symlink() or not archive.is_file() or archive.stat().st_size > 50 * 1024 * 1024:
                    continue
            ...
            with zipfile.ZipFile(io.BytesIO(raw)) as package:
                code = package.read('launch038_home.py')
            exec(compile(code, 'launch038_home.py', 'exec'),
