#!/usr/bin/env python3
"""Install the verified042 outer ZIP under the existing WSL lab and run its offline verification workflow.
...
It uses no network itself; the versioned workflow checks the design and publishes its full report plus receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_042'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 64 << 20, 32 << 20, 128 << 20, 512
ROOT_FILES = {'SHA256SUMS', 'package_release.json', 'launch042_home.py', 'README_042.md'}
BOOT_SCRIPTS = {'scripts/checkpoint042_workflow.py', 'scripts/stage_source_packet038.py'}
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Checking the042 design offline and returning its full report plus receipt to GitHub. No model or experiment is started.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'scripts/checkpoint042_workflow.py'),
                      '--lab', str(lab), '--bundle', str(bundle)], cwd=lab)
    require(result.returncode == 0, 'Offline verification workflow stopped; the same command safely retries verification or receipt publication')
    return bundle
