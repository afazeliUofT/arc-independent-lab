"""Local, deterministic checkpoint-030 ZIP handoff; no network or native launch.

With ZIP and --sha256, default behavior is read-only inspection. --apply installs
and stages only recognized transitions; --verify requires the complete selected
variant. The release is explicitly built as correction or publication-only.
In correction mode, --approve-correction selects the exact pinned approval, changing only ESCALATION.md and CHECKPOINT_030.json from the unsigned
ZIP. The user separately commits, pushes, and launches the attended controller.
No arguments prints inspection guidance without changing anything.
"""
import argparse
import datetime
import hashlib
...
BASE = "b2f47e9442cbc5bd06490dd0d9fb0475a860aae9"
...
CHECKPOINT_ID = "P3_CHECKPOINT_030"
CHECKPOINT_PATH = "state/CHECKPOINT_030.json"
ESCALATION_PATH = "state/ESCALATION.md"
SCOPE_PATH = "configs/P3_FINITE_REVIEW_SCOPE_030.json"
APPROVAL_ID = "APPROVE_P3_FINITE_REVIEW_030_CORRECTION"
RELEASE_MODE = "correction"
SCOPE_SHA256 = "5abc5794f3adadeebc0e3df3e1ffd5bc4416806d2d1212f0a6b73db96d177060"
UNSIGNED_ESCALATION_SHA256 = "a8b8fd9e1e608aa3c715e454beb4f9929f9a68c3bcf147b2d0731b22d91ae2bd"
...
def approval_metadata(approved=False):
    if RELEASE_MODE != "correction":
        raise ValueError("This publication does not propose a native approval")
    return {
        "status": "recorded_by_explicit_human_flag" if approved else "pending_explicit_human_flag",
        "approval_id": APPROVAL_ID,
        "scope_path": SCOPE_PATH,
        "scope_sha256": SCOPE_SHA256,
        "unsigned_escalation_sha256": UNSIGNED_ESCALATION_SHA256,
        "method": "checkpoint030_handoff.py --approve-correction",
    }
...
def validate_unsigned(files):
    if RELEASE_MODE not in ("correction", "publication-only"):
        raise ValueError("Checkpoint030 release mode has not been finalized")
    if not valid_hash(UNSIGNED_ESCALATION_SHA256):
        raise ValueError("Checkpoint030 unsigned request pin has not been finalized")
    for name in (ESCALATION_PATH, CHECKPOINT_PATH):
        if name not in files:
            raise ValueError("Required unsigned payload file absent: " + name)
    raw = files[ESCALATION_PATH]["body"]
    if sha(raw) != UNSIGNED_ESCALATION_SHA256:
        raise ValueError("Unsigned request differs from its fixed hash")
    if any(line.strip() == "## ANSWER" for line in raw.decode("utf-8").splitlines()):
        raise ValueError("The unsigned request already contains an ANSWER heading")
    checkpoint, rows = checkpoint_rows(files[CHECKPOINT_PATH]["body"])
    if checkpoint.get("release_mode") != RELEASE_MODE:
        raise ValueError("Checkpoint release mode differs from the fixed release")
    if RELEASE_MODE == "correction":
        if not valid_hash(SCOPE_SHA256) or SCOPE_PATH not in files:
            raise ValueError("Correction scope pin or payload is absent")
        if sha(files[SCOPE_PATH]["body"]) != SCOPE_SHA256:
            raise ValueError("Correction scope differs from its fixed hash")
        if checkpoint.get("correction_approval") != approval_metadata():
            raise ValueError("Unsigned approval metadata differs from the fixed request")
    elif checkpoint.get("correction_approval") is not None or not checkpoint.get("publication_only"):
        raise ValueError("Publication-only release must not include approval metadata")
    for name, entry in files.items():
        if name == CHECKPOINT_PATH:
            continue
        if (name not in rows or rows[name]["sha256"] != entry["sha256"]
                or rows[name]["mode"] != entry["mode"]):
            raise ValueError("Checkpoint manifest differs from unsigned payload: " + name)
    return checkpoint
...
def select_variant(unsigned_files, approve_correction=False):
    """Derive bytes in memory only; never record approval unless flag is explicit."""
    validate_unsigned(unsigned_files)
    selected = {name: dict(entry) for name, entry in unsigned_files.items()}
    if not approve_correction:
        return selected
    if RELEASE_MODE != "correction":
        raise ValueError("Publication-only release cannot record correction approval")
    request = unsigned_files[ESCALATION_PATH]["body"].rstrip(b"\n")
    answer = ("\n\n## ANSWER\n" + APPROVAL_ID + "\nscope_sha256: " + SCOPE_SHA256 + "\n").encode()
    selected[ESCALATION_PATH]["body"] = request + answer
    selected[ESCALATION_PATH]["sha256"] = sha(request + answer)
    # Everything explanatory stays before ANSWER. The controller requires these
    # two exact answer lines and no prose after them.
    checkpoint, rows = checkpoint_rows(unsigned_files[CHECKPOINT_PATH]["body"])
    rows[ESCALATION_PATH]["sha256"] = selected[ESCALATION_PATH]["sha256"]
    checkpoint["correction_approval"] = approval_metadata(approved=True)
    selected[CHECKPOINT_PATH]["body"] = json_bytes(checkpoint)
    selected[CHECKPOINT_PATH]["sha256"] = sha(selected[CHECKPOINT_PATH]["body"])
    changed = {name for name in selected if selected[name]["body"] != unsigned_files[name]["body"]}
    if changed != {ESCALATION_PATH, CHECKPOINT_PATH}:
        raise ValueError("Approval variant must change exactly the two declared state files")
    return selected
...
def classify_head(git, head, expected):
    ...
    if (git.text("rev-list", "--parents", "-n", "1", head).split() == [head, BASE]
        ...
        return "checkpoint030"
    raise ValueError("HEAD is neither the pinned base029 nor the exact selected030 child")
...
def inspect(root, files, complete=False, unsigned_files=None, quiet=False):
    ...
    complete = complete or disposition == "checkpoint030"
    index = git.index()
    ...
    if not quiet:
        ...
        print("Verified complete staged checkpoint." if complete else "Verified known base029/030 transitions.")
...
def apply(root, files, unsigned_files=None):
    ...
    backup_parent = ordinary(root, "delivery/checkpoint030_backups", directory=True)
...
def main():
    ...
    parser.add_argument("--approve-correction", action="store_true", help=
                        "Explicitly approve the exact scope030 pinned in this release; "
                        "publication remains separate from any native reviewer launch")
    args = parser.parse_args()
    if args.zip_path is None and args.sha256 is None and not (args.apply or args.verify or args.approve_correction):
        print("Inspection only. Supply the delivered ZIP and --sha256 to inspect exact local transitions.")
        print("--apply installs/stages; --approve-correction explicitly selects the approval variant.")
        print("No files, Git configuration, credentials, or network state were changed.")
        return
    if args.zip_path is None or args.sha256 is None:
        parser.error("ZIP path and --sha256 are both required")
    unsigned_files = load_package(args.zip_path, args.sha256)
    files = select_variant(unsigned_files, args.approve_correction)
    root = Path.home() / "ARC_Independent_Lab"
    print("Target:", root)
    print("Pinned base:", BASE)
    print("Selected variant:", "explicit correction approval" if args.approve_correction else "unsigned request")
    if args.approve_correction:
        print("Exact correction approval:", APPROVAL_ID)
        print("Scope SHA-256:", SCOPE_SHA256)
        print("Read state/ESCALATION.md for the concrete controls and finite numerical limits.")
        print("Selecting this approval does not start a native client or model.")
    for name, entry in sorted(files.items()):
        print(name, entry["old_sha256"] or "absent", "->", entry["sha256"])
    if args.apply:
        apply(root, files, unsigned_files=unsigned_files)
    else:
        inspect(root, files, complete=args.verify, unsigned_files=unsigned_files)
