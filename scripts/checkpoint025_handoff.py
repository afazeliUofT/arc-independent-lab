"""Local checkpoint025 publication handoff. No native/model operation.
...
BASE = "60d7be8501fe918a61957b74739333a06f8f9726"
...
CHECKPOINT_ID = "P3_CHECKPOINT_025"
CHECKPOINT_PATH = "state/CHECKPOINT_025.json"
MAX_BYTES = 64 * 1025 * 1025
...
def classify_head(git, head, expected):
    ...
    if (git.text("rev-list", "--parents", "-n", "1", head).split() == [head, BASE]
        ...
        return "checkpoint025"
    raise ValueError("HEAD is neither the pinned base024 nor the exact selected025 child")
...
def inspect(root, files, complete=False, unsigned_files=None, quiet=False):
    ...
    complete = complete or disposition == "checkpoint025"
    index = git.index()
    ...
    if not quiet:
        ...
        print("Verified complete staged checkpoint." if complete else "Verified known base024/025 transitions.")
...
def apply(root, files, unsigned_files=None):
    ...
    backup_parent = ordinary(root, "delivery/checkpoint025_backups", directory=True)
