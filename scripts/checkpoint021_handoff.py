"""Local P3 checkpoint-021 ZIP handoff. No commit, push, fetch, or authentication.
...
BASE = "b6a57210fcbf86d29a6e8d788c7b8cf8bf4d7689"
...
def load_package(path, expected_hash):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if (set(manifest) != {"checkpoint_id", "base_commit", "files"}
                or manifest["checkpoint_id"] != "P3_CHECKPOINT_021"
...
def apply(root, files):
    ...
    if head != BASE:
        ...
        return
    backup_parent = ordinary(root, "delivery/checkpoint021_backups", directory=True)
