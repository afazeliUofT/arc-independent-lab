"""Local, deterministic checkpoint-024 ZIP handoff; no network or native launch.
...
approval, changing only ESCALATION.md and CHECKPOINT_024.json from the unsigned
...
BASE = "b0a46da67e18212be58764ac525eb693d8eea55e"
...
CHECKPOINT_ID = "P3_CHECKPOINT_024"
CHECKPOINT_PATH = "state/CHECKPOINT_024.json"
ESCALATION_PATH = "state/ESCALATION.md"
SCOPE_PATH = "configs/P3_FINITE_REVIEW_SCOPE_024.json"
APPROVAL_ID = "APPROVE_P3_FINITE_REVIEW_024_CORRECTION"
SCOPE_SHA256 = "bdf0d9cb689793656c8616f72f2f1a6a45a6bc4a6cf5abed3e0e0e4dde8ee0db"
UNSIGNED_ESCALATION_SHA256 = "352309d0f945ef5f16046163460e179fe48192cfc62134e87be7daca2f188e1f"
...
def approval_metadata(approved=False):
    return {
        ...
        "method": "checkpoint024_handoff.py --approve-correction",
    }
...
def validate_unsigned(files):
    if not valid_hash(SCOPE_SHA256) or not valid_hash(UNSIGNED_ESCALATION_SHA256):
        raise ValueError("Checkpoint024 build pins have not been finalized")
...
def classify_head(git, head, expected):
    ...
    if (git.text("rev-list", "--parents", "-n", "1", head).split() == [head, BASE]
        ...
        return "checkpoint024"
    raise ValueError("HEAD is neither the pinned base023 nor the exact selected024 child")
...
def inspect(root, files, complete=False, unsigned_files=None, quiet=False):
    ...
    complete = complete or disposition == "checkpoint024"
    index = git.index()
    ...
    if not quiet:
        ...
        print("Verified complete staged checkpoint." if complete else "Verified known base023/024 transitions.")
...
def apply(root, files, unsigned_files=None):
    ...
    backup_parent = ordinary(root, "delivery/checkpoint024_backups", directory=True)
...
def main():
    ...
    parser.add_argument("--approve-correction", action="store_true", help=
                        "Explicitly approve scope024: cumulative ceilings 6 native starts / 3 turns; "
                        "4 starts / 1 turn already consumed, at most 2 additional starts / 2 turns")
    ...
    if args.approve_correction:
        print("Scope024: old cumulative ceiling 5 starts / 2 turns; corrected ceiling 6 starts / 3 turns.")
        print("Already consumed: 4 starts / 1 turn. Additional allowance: at most 2 starts / 2 turns.")
        print("Sol Max, existing access restrictions, and no-spending terms remain unchanged.")
    for name, entry in sorted(files.items()):
