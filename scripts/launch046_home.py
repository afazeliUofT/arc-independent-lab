#!/usr/bin/env python3
"""Install the verified046 outer ZIP under the existing WSL lab and run its consolidated resource profile workflow.
...
It uses no network itself; the versioned workflow runs the bounded fabricated full-work resource profile once and publishes its report plus receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_046'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 64 << 20, 32 << 20, 128 << 20, 512
ROOT_FILES = {'SHA256SUMS', 'package_release.json', 'launch046_home.py', 'README_046.md'}
BOOT_SCRIPTS = {'scripts/checkpoint046_workflow.py', 'scripts/stage_source_packet038.py'}
...
def archive_payloads(raw):
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        ...
        require(all(name in ROOT_FILES or
                    (name.startswith('public/') and PurePosixPath(name).suffix == '.sha256') or
                    (name.startswith(('public/', 'scripts/', 'private/')) and PurePosixPath(name).suffix in
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Running consolidated development resource profiling once. Preserving all043/044 measurements and045 diagnostics and returning the full available report and receipt to GitHub.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'scripts/checkpoint046_workflow.py'),
                      '--lab', str(lab), '--bundle', str(bundle)], cwd=lab)
    require(result.returncode == 0, 'Report publication or repository checks did not complete; the same command reuses saved execution state and retries publication without rerunning a reserved profile invocation')
    return bundle
