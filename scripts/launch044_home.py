#!/usr/bin/env python3
"""Install the verified044 outer ZIP under the existing WSL lab and run its development-profile workflow.
...
It uses no network itself; the versioned workflow continues the remaining
development rows once and publishes its report plus receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_044'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 64 << 20, 32 << 20, 128 << 20, 512
ROOT_FILES = {'SHA256SUMS', 'package_release.json', 'launch044_home.py', 'README_044.md'}
BOOT_SCRIPTS = {'scripts/checkpoint044_workflow.py', 'scripts/stage_source_packet038.py'}
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Preserving six completed043 measurements and continuing the remaining21 rows under one044 reservation. Returning the full available report and receipt to GitHub.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'scripts/checkpoint044_workflow.py'),
                      '--lab', str(lab), '--bundle', str(bundle)], cwd=lab)
    require(result.returncode == 0, 'Report publication or repository checks did not complete; the same command reuses saved execution state and retries publication without rerunning a reserved profile')
    return bundle
