#!/usr/bin/env python3
"""Install the verified043 outer ZIP under the existing WSL lab and run its development-profile workflow.
...
It uses no network itself; the versioned workflow runs one bounded development profile and publishes its report plus receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_043'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 64 << 20, 32 << 20, 128 << 20, 512
ROOT_FILES = {'SHA256SUMS', 'package_release.json', 'launch043_home.py', 'README_043.md'}
BOOT_SCRIPTS = {'scripts/checkpoint043_workflow.py', 'scripts/stage_source_packet038.py'}
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Running the one reserved043 development profile and returning its report plus receipt to GitHub. No model or target experiment is started.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'scripts/checkpoint043_workflow.py'),
                      '--lab', str(lab), '--bundle', str(bundle)], cwd=lab)
    require(result.returncode == 0, 'Development-profile workflow stopped; the same command reuses the reserved execution state and retries report publication without rerunning the profile')
    return bundle
