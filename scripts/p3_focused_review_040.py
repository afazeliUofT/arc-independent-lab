#!/usr/bin/env python3
"""Checkpoint040: one finite attended source-aware second-audit review.
...
032 semantics. The scientific packet, broker/schema and prompt remain frozen039.
Only exact dynamic-tool admission, pure payload preflight and accounting scope
...
SCOPE_PATH = 'configs/P3_FOCUSED_REVIEW_SCOPE_040.json'
SCOPE_SHA256 = '682521fe5c47ffbd620064655bb8d9b1f6d365d5d84cac5a1e50ee72d0172f4e'
RUN_NAME = 'P3_FOCUSED_REVIEW_040'
REPORT_KIND = 'P3_FOCUSED_REVIEW_040_v1'
...
def run_stage(scope, prior_scope, run, mode, packet, manifest_sha, expected_binding=None):
    ...
    session = module('p3_reviewer_session_040')
    ...
    plan.update({'kind': 'P3_FOCUSED_REVIEW_NATIVE_MOUNT_PLAN_040_v1',
...
def collect_receipts(run, attempted_modes):
    ...
    try:
        result = module('p3_receipt_collection_032').collect(run,
            emitter_path=ROOT / 'scripts/p3_reviewer_session_040.py',
...
def accounting(observations, attempted_modes):
    ...
    return {'prior_native_starts':16,'prior_sent_turns':12,
        'observed_native_starts_this_attempt':starts,'observed_sent_turns_this_attempt':turns,
        'cumulative_observed_native_starts':16 + starts,'cumulative_observed_sent_turns':12 + turns,
        'reserved_native_starts_this_attempt':2,'reserved_turns_this_attempt':2,
        'cumulative_native_ceiling':18,'cumulative_sent_turn_ceiling':14,
        'maximum_cumulative_reservation_native_starts':18,'maximum_cumulative_reservation_turns':14,
...
def load_bundle():
    ...
    require(sha(raw) == SCOPE_SHA256, '040 scope differs from sealed release')
    scope = json.loads(raw)
    require(scope.get('scope_id') == 'P3_FOCUSED_REVIEW_SCOPE_040' and
            scope.get('report_directory') == 'delivery/' + RUN_NAME,
            '040 scope identity differs')
    sources = scope.get('source_pins')
    require(type(sources) is dict and len(sources) >= 20, 'Incomplete040 source closure')
    for name, digest in sources.items():
        ...
        require(sha(bounded(ROOT / name)) == digest, '040 source dependency changed')
    ...
    require(all(canonical(scope.get(k)) == canonical(baseline[k]) for k in fixed),
            '040 altered unchanged native controls')
    counts = {'native_starts_observed':16,'explicit_model_turns_observed':12,
              'additional_native_processes_maximum':2,'additional_explicit_model_turns_maximum':2,
              'cumulative_native_starts_maximum':18,'cumulative_explicit_model_turns_maximum':14,
    ...
    require(canonical(scope.get('prior_attempt_accounting')) == canonical(counts),
            '040 cumulative accounting differs')
    ...
    require(scope['private_packet_directory'] == 'delivery/P3_FOCUSED_REVIEW_PACKET_040',
    ...
    require(auth['path'] == 'state/authorizations/P3_FOCUSED_REVIEW_040_STANDING_AUTHORIZATION.md' and
...
def prior_history(scope):
    ...
    returned = records['review039']
    counts039 = returned.get('attempt_accounting', {})
    require(returned.get('status') == 'STOPPED_WITHOUT_COMPLETED_REVIEW' and
            returned.get('reviewer_output') is None and
            counts039.get('prior_native_starts') == 14 and
            counts039.get('prior_sent_turns') == 11 and
            counts039.get('observed_native_starts_this_attempt') == 2 and
            counts039.get('observed_sent_turns_this_attempt') == 1 and
            counts039.get('cumulative_observed_native_starts') == 16 and
            counts039.get('cumulative_observed_sent_turns') == 12 and
            counts039.get('missing_session_counts_are_only_minimum_observations') is False,
            'Prior039 exact returned accounting differs')
    for mode in ('synthetic', 'science'):
        session = records[mode + '039']
        stage = records[mode + '_stage039']
        require(session.get('kind') == 'P3_FINITE_REVIEWER_SESSION_039_v1' and
                session.get('mode') == mode and
                session.get('client_started') is True and
                session.get('native_process_reaped') is True and
                session.get('verdict_submitted') is False and
                stage.get('stage') == mode and stage.get('observation') == session and
                stage.get('failures') == ([] if mode == 'synthetic' else
                    [{'phase':'session','code':'required_outcome_not_observed'}]) and packet_admitted(stage) and
                stage.get('host_checks', {}).get('all_noncredential_checks_pass') is True,
                'Prior039 source-bound original stage differs')
    canary, science = records['synthetic039'], records['science039']
    require(canary.get('status') == 'SYNTHETIC_REFUSAL_OBSERVED' and
            canary.get('model_turn_request_sent') is True and
            synthetic_recovery_admitted(canary) and
            science.get('status') == 'STOPPED_WITHOUT_VERDICT' and
            science.get('thread_request_sent') is False and
            science.get('model_turn_request_sent') is False and
            science.get('thread_admission') is None and
            science.get('admission_reason') == 'Unexpected reviewer dynamic tool spec',
            'Prior039 failure is not the confirmed pre-thread tool-contract stop')
    ...
    return {'prior_native_starts':16,'prior_sent_turns':12,
            ...
            'actual039_original_receipts_reverified':True,
            'prior039_science_thread_or_model_turn_started':False,
            'prior039_verdict_available':False,
...
def dynamic_tool_preflight(scope):
    """Exercise real frozen scientific tools through real admission, offline.

    This exact code path previously failed only after spending a canary turn.
    Construct both actual requested profiles/payloads before any native start.
    Runtime observations, catalog/entitlement and source delivery still require
    their unchanged live checks; payload construction is not a runtime receipt.
    """
    broker = module('p3_focused_review_broker_039')
    admission = module('p3_reviewer_admission_040')
    profile = module('p3_reviewer_profile_022')
    preflight = module('gate0_client_preflight')
    schema = bounded(ROOT / 'artifacts/GATE0_CODEX_INTERFACE/20260907_001/config-schema.json')
    full_specs = broker.dynamic_tool_specs()
    observations = {}
    for mode in ('synthetic', 'science'):
        run = ROOT / 'delivery' / RUN_NAME / (mode + '_native')
        requested = profile.reviewer_overrides(preflight.overrides(run), schema)
        specs = ([row for row in full_specs if row['name'] == 'read_text']
                 if mode == 'synthetic' else full_specs)
        params = admission.thread_params(BINDING['model'], BINDING['effort'],
                                         str(run), specs, requested)
        require(params['dynamicTools'] == specs and params['permissions'] == ':read-only' and
                params['environments'] == [] and params['runtimeWorkspaceRoots'] == [] and
                params['selectedCapabilityRoots'] == [] and params['ephemeral'] is True and
                params['approvalPolicy'] == 'never' and params['allowProviderModelFallback'] is False,
                'Pure dynamic-tool payload boundary differs')
        observations[mode] = {
            'tool_names':[row['name'] for row in specs],
            'frozen039_tool_contract_sha256':admission.FROZEN039_TOOL_CONTRACT_SHA256[mode],
            'real_thread_params_admitted':True,
            'requested_profile_preserved':params['config'] == {**requested, 'model_reasoning_effort':BINDING['effort']}}
    return {'kind':'P3_FOCUSED_DYNAMIC_TOOL_PREFLIGHT_040_v1',
            'modes':observations, 'native_clients_started':0, 'model_turns_sent':0,
            'runtime_catalog_or_permissions_observed':False,
            'scientific_packet_or_verdict_changed':False}
...
def packet_preflight(scope, packet):
    ...
    with tempfile.TemporaryDirectory(prefix='P3_040_packet_check_', dir=ROOT / 'delivery') as tmp:
...
def execute(scope, prior_scope, pins, packet):
    ...
    tool_preflight = dynamic_tool_preflight(scope)
    ...
    exclusive_json(run / 'ATTEMPT.json', {'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'prior_native_starts':16,'prior_sent_turns':12,
    ...
    result = {'kind':REPORT_KIND,'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'dynamic_tool_preflight':tool_preflight,
    ...
    try:
        ...
        require(dynamic_tool_preflight(scope) == tool_preflight,
                'Dynamic-tool payload changed before science')
...
def existing(run, pins):
    ...
    if value.get('status') == 'REVIEWER_OUTPUT_PRESERVED':
        require(admitted_synthetic(observed['stages'].get('synthetic')) and
                admitted_science(observed['stages'].get('science')), 'Existing040 admission no longer holds')
        output = value.get('reviewer_output',{})
        require(output.get('sha256') == sha(bounded(run / 'science_output/REVIEW_VERDICT.json')),
                'Existing040 reviewer output changed')
    return run / 'REPORT.json'
    ...
    try:
        ...
        if not args.run_attended_review:
            print(json.dumps({'status':'SEALED_SCOPE_INSPECTED','scope_sha256':SCOPE_SHA256,
                              'dynamic_tool_preflight':dynamic_tool_preflight(scope),
