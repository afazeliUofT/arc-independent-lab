"""Local P3 checkpoint-015 ZIP handoff. No commit, push, fetch, or authentication.
...
BASE = "82cd17dd99bbbd51484540c6288ca98148b87903"
...
def load_package(path, expected_hash):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if (set(manifest) != {"checkpoint_id", "base_commit", "files"}
                or manifest["checkpoint_id"] != "P3_CHECKPOINT_015"
...
def apply(root, files):
    ...
    if head != BASE:
        ...
        return
    backup_parent = ordinary(root, "delivery/checkpoint015_backups", directory=True)
