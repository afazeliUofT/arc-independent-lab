#!/usr/bin/env python3
"""Collect the fixed, noncredential 039 evidence files; never run Git or a model.
...
VERSION = "P3_REVIEW_039_EVIDENCE_COLLECTION_039_v1"
...
ARCHIVE_PARENT = "artifacts/P3_REVIEW_039_RETURN"
...
def _summary(entries):
    ...
    if any(value == "REFUSED" for value in states.values()):
        ...
        collection = "WORKFLOW_EVIDENCE_ONLY_NO_039_RECEIPTS"
    elif report != "PRESENT":
    ...
    return {"collection_status": collection, "main_report": report,
            ...
            "original039_file_count": original_count,
            ...
            "actual_usage": "READ_SESSION_AND_CONTROLLER_RECEIPTS; NEVER_INFER_ZERO_FROM_MISSING_FILES",
            "missing_files_do_not_establish_no_execution": True}



def bulk_copy_guard(root_fd, verdict):
    """Refuse a long exact private-source passage, without altering the verdict.

    This 200-word screen is a narrow accidental bulk-copy check, not a copyright
    classifier or proof that all shorter quotations are acceptable. Missing
    source texts make the screen explicitly unavailable, not a false pass.
    """
    size = 200
    strings = []
    pending = [verdict]
    ...
    for value in strings:
        ...
        candidate_windows.update(sha256(' '.join(words[i:i + size]).encode('ascii'))
                                 for i in range(len(words) - size + 1))
    source_rows = []
    detected = False
    for source_id in ('CHV79', 'PK97'):
        name = 'delivery/P3_FOCUSED_REVIEW_PACKET_039/reading/' + source_id + '/full_text.txt'
        try:
            raw, _, _ = _read_fixed(root_fd, name, 32 * MIB)
            words = re.findall(r"[a-z0-9]+", raw.decode('utf-8').lower())
            match = any(sha256(' '.join(words[i:i + size]).encode('ascii')) in candidate_windows
                        for i in range(len(words) - size + 1)) if candidate_windows else False
            detected = detected or match
            source_rows.append({'source_id': source_id, 'sha256': sha256(raw), 'status': 'CHECKED',
                                'long_exact_overlap_detected': match})
        except (OSError, CollectionError, UnicodeError):
            source_rows.append({'source_id': source_id, 'status': 'UNAVAILABLE'})
    return {'normalized_contiguous_word_threshold': size, 'detected': detected,
            'status': 'BULK_COPY_REFUSED' if detected else
                      'CHECKED_NO_LONG_EXACT_OVERLAP' if all(row['status'] == 'CHECKED' for row in source_rows)
                      else 'SOURCE_SCREEN_INCOMPLETE',
            'shorter_quotations_and_semantic_copying_not_certified': True,
            'source_text_saved_or_published': False, 'sources': source_rows}
        ...
        for relative, limit in SOURCE_LIMITS.items():
            ...
            try:
                ...
                if relative.endswith(".json"):
                    try:
                        parsed = json.loads(raw.decode("utf-8"), parse_constant=lambda value: (_ for _ in ()).throw(ValueError("nonfinite")))
                        if type(parsed) is not dict:
                            raise ValueError("nonobject")
                    except (ValueError, UnicodeError, RecursionError):
                        raise CollectionError("invalid_json_object_refused")
                    if relative == MAIN + "science_output/REVIEW_VERDICT.json":
                        entry["private_source_bulk_copy_guard"] = bulk_copy_guard(root_fd, parsed)
                        if entry["private_source_bulk_copy_guard"]["detected"]:
                            raise CollectionError("long_exact_private_source_passage_refused")
                elif relative.endswith(".sha256") and not re.fullmatch(rb"[0-9a-f]{64}\n?", raw):
                    raise CollectionError("invalid_report_hash_refused")
