#!/usr/bin/env python3
"""Install the verified041 outer ZIP under the existing WSL lab and run its offline verification workflow.
...
It uses no network itself; the versioned workflow verifies frozen evidence and publishes one receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_041'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 64 << 20, 32 << 20, 128 << 20, 512
ROOT_FILES = {'SHA256SUMS', 'package_release.json', 'launch041_home.py', 'README_041.md'}
BOOT_SCRIPTS = {'scripts/checkpoint041_workflow.py', 'scripts/stage_source_packet038.py'}
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Verifying frozen040 evidence offline and returning the041 receipt to GitHub. No model or experiment is started.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'scripts/checkpoint041_workflow.py'),
