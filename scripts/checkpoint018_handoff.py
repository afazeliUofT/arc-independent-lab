"""Local P3 checkpoint-018 ZIP handoff. No commit, push, fetch, or authentication.
...
BASE = "e0cd916249990c1a5993a8adefba12c893f9c60d"
...
def load_package(path, expected_hash):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if (set(manifest) != {"checkpoint_id", "base_commit", "files"}
                or manifest["checkpoint_id"] != "P3_CHECKPOINT_018"
...
def apply(root, files):
    ...
    if head != BASE:
        ...
        return
    backup_parent = ordinary(root, "delivery/checkpoint018_backups", directory=True)
