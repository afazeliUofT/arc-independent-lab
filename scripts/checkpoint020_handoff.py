"""Local P3 checkpoint-020 ZIP handoff. No commit, push, fetch, or authentication.
...
BASE = "9006972e4084fde2580c83ff3c9942fbca2bb52d"
...
def load_package(path, expected_hash):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if (set(manifest) != {"checkpoint_id", "base_commit", "files"}
                or manifest["checkpoint_id"] != "P3_CHECKPOINT_020"
...
def apply(root, files):
    ...
    if head != BASE:
        ...
        return
    backup_parent = ordinary(root, "delivery/checkpoint020_backups", directory=True)
