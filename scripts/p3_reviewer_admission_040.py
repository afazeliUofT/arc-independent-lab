...
Version040 changes only the dynamic-tool contract to the exact frozen039
scientific list or its unchanged read_text-only synthetic subset.
"""
...
_profile_module = _module("p3_reviewer_profile_022")
...
MAX_REVIEW_EFFORT = "max"
FROZEN039_TOOL_CONTRACT_SHA256 = {'science': '56a093ba1366ec36ded677f91ae056d2a782e16d427645d1e0287bfa72d515c4', 'synthetic': '136f7b73de4228979d61715a4673b091a7fcd8dcc9ab5781d5cf5761eb302645'}
...
def select_model(catalog):
    """Choose only exact Sol with its explicitly approved advertised Max effort.
    ...
    require(MAX_REVIEW_EFFORT in supported,
            "Requested exact Max reviewer effort is not advertised")
    return {"model": REQUESTED_MODEL, "id": _identifier(row.get("id")),
            "effort": MAX_REVIEW_EFFORT}


def _model_effort(model, effort):
    require(model == REQUESTED_MODEL and type(model) is str,
            "Reviewer model fallback denied")
    require(type(effort) is str and effort == MAX_REVIEW_EFFORT,
            "Reviewer effort differs from the exact approved Max effort")
...
def thread_params(model, effort, cwd, dynamic_tools, profile):
    ...
    allowed = {"read_text", "read_page_image", "hash_file", "source_access_receipt", "submit_verdict"}
    names = []
    ...
    # Exact canonical JSON binds names, descriptions and every nested schema.
    # Only the original039 full science list and its read_text-only canary are
    # admitted; no observer operation, arbitrary subset or schema relaxation.
    try:
        encoded = json.dumps(dynamic_tools, sort_keys=True, separators=(",", ":"),
                             ensure_ascii=True, allow_nan=False).encode("utf-8")
    except (TypeError, ValueError, UnicodeError, RecursionError):
        raise Stop("Reviewer dynamic tool contract is not canonical JSON") from None
    require(len(encoded) <= 256 * 1024, "Reviewer dynamic tool contract exceeds bound")
    require(hashlib.sha256(encoded).hexdigest() in FROZEN039_TOOL_CONTRACT_SHA256.values(),
            "Reviewer dynamic tools differ from frozen039 contract")
