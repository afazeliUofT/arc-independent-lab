#!/usr/bin/env python3
"""Human-run publication only of declared checkpoint034 documents and evidence.
...
BASE = '296cf94b9e692f4ddbee5c554fc87aaa5a298e60'
REMOTE = 'https://github.com/afazeliUofT/arc-independent-lab.git'
META = 'delivery/P2_034_PUBLICATION'
...
def publish_pending(root, pending):
    ...
    if result.returncode:
        print('PUSH FAILED: local commit and pending publication preserved. Repeat the same034 command; it retries publication only.', flush=True)
        return result.returncode
    receipt = {'kind':'P2_034_PUBLISHED_v1', 'commit':current,
...
def install(root, bundle, manifest):
    ...
    for name, body in payload.items():
        ...
        tmp = p.with_name(p.name + '.034-install')
...
def package(root, bundle):
    ...
    if (manifest.get('kind') != 'P2_CHECKPOINT_034_PACKAGE_v1'
...
def drive(root, bundle):
    ...
    if os.path.lexists(pending_path):
        ...
        expected_rows = rows
        if (pending.get('kind') != 'P2_034_PENDING_PUBLICATION_v1'
    ...
    if os.path.lexists(done_path):
        ...
        expected_rows = rows
        if (done.get('kind') != 'P2_034_PUBLISHED_v1' or done.get('files') != expected_rows
    ...
    pending = {'kind':'P2_034_PENDING_PUBLICATION_v1', 'parent':BASE,
...
COMMIT_MESSAGE = 'Phase 2: preserve second-pass raw generation and checkpoint033 verification'


def main():
    if sys.argv[1:] != ['--publish']:
        raise SystemExit('Use --publish for checkpoint034 document publication only.')
...
if __name__ == '__main__':
    try:
        ...
        print('Publication interrupted. Existing files are preserved; repeat the same034 command.', file=sys.stderr)
