#!/usr/bin/env python3
"""One manual compatibility correction within the existing finite-review scope.
...
mutation is implemented. Both failed startup attempts and all historical controllers remain unchanged.
The existing approved scope, model limits and host access are unchanged.
"""
...
REQUIRED_CORRECTION_PINS = frozenset({
    'scripts/p3_finite_review_020.py',
    'scripts/p3_reviewer_session_021.py',
    'scripts/p3_reviewer_admission_021.py',
    'artifacts/P3_FINITE_REVIEW_CORRECTION_OBSERVATIONS/20260909_001/REPORT.json',
    'artifacts/P3_CONFIG_ORIGIN_SOURCE/20260909_001/MANIFEST.json',
})
RUN_NAME = 'P3_FINITE_REVIEW_019_REPAIR_002'
REPORT_KIND = 'P3_FINITE_REVIEW_019_REPAIR_002_v1'
PRIOR_REPORT_SHA256 = 'd9e52a7c7401651e482945c9d30786abde6f63d10fcb2412c7b92017b42635e7'
...
def run_stage(scope, prior_scope, run, mode, packet, manifest_sha, expected_binding=None):
    ...
    session = module('p3_reviewer_session_021')
...
def prior_failed_attempt():
    """Recheck both preserved attempts without native, log or auth operations.

    Historical validators verify their own exact dependencies and local session
    files. The uploaded second receipt is independently pinned here. Its record
    of the original attempt must equal a fresh read-only verification of that
    original receipt, so a prior model turn cannot disappear from the count.
    """
    correction = module('p3_finite_review_020')
    try:
        _, _, correction_pins = correction.load_bundle()
        path = correction.existing(ROOT / 'delivery' / correction.RUN_NAME, correction_pins)
        require(path is not None, 'The preserved020 correction attempt is required')
        raw = bounded(path)
        require(sha(raw) == PRIOR_REPORT_SHA256,
                'Correction020 report differs; preserve it and stop')
        original_attempt = correction.prior_failed_attempt()
    except (correction.Stop, OSError, ValueError, KeyError, TypeError):
        raise Stop('Prior019/020 receipt, pins or session files differ; preserve them') from None
    ...
    require(type(stages) is list and len(stages) == 1 and
            ...
            'Prior020 stage or completion differs; no correction admitted')
    ...
    require(type(observed) is dict and
            ...
            'Prior020 operation or cleanup differs; no correction admitted')
    require(type(original_attempt) is dict and
            original_attempt.get('prior_failed_native_processes') == 1 and
            original_attempt.get('prior_explicit_model_turns') == 0 and
            original_attempt.get('access_scope_sha256') == SCOPE_SHA256 and
            value.get('prior_attempt') == original_attempt,
            'Prior019/020 cumulative receipt differs; no correction admitted')
    return {'report_path':str(path.relative_to(ROOT)), 'report_sha256':sha(raw),
            'unchanged_correction020_receipt_verified':True,
            'original_attempt':original_attempt,
            'prior_explicit_model_turns':0, 'prior_failed_native_processes':2,
            'both_prior_attempts_preserved':True,
            'access_scope_sha256':SCOPE_SHA256,
            'correction_requires_explicit_human_launch':True,
            'corrective_invocation_native_processes_maximum':2,
            'cumulative_native_processes_maximum_including_failed_attempts':4,
...
def execute(scope, prior_scope, pins, authorization, packet_info):
    ...
    result = {'kind':REPORT_KIND,'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'correction':'Source-derived configuration origins and safe control diagnostics; scope unchanged'}
    try:
...
def main():
    ...
    if not args.run_attended_review:
        print('Existing approved scope SHA256: '+SCOPE_SHA256)
        print('Manual correction preserves both019/020 attempts and uses a new report folder.')
