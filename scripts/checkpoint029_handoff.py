"""Local deterministic checkpoint-029 handoff; no network or native/model launch.

ZIP plus --sha256 defaults to read-only inspection. --apply installs/stages only
recognized transitions; --verify requires the complete checkpoint. No approvals
or model allowances are created. The user separately executes the publication
wrapper to commit and push this forensic checkpoint.
"""
import argparse
import datetime
import hashlib
...
BASE = "309b2330819b55fc5573e57163d4ae6e09c5d21d"
...
CHECKPOINT_ID = "P3_CHECKPOINT_029"
CHECKPOINT_PATH = "state/CHECKPOINT_029.json"
...
def validate_checkpoint(files):
    if CHECKPOINT_PATH not in files:
        raise ValueError("Required checkpoint payload absent")
    checkpoint, rows = checkpoint_rows(files[CHECKPOINT_PATH]["body"])
    if (checkpoint.get("publication_only") is not True
            or checkpoint.get("new_approval_required") is not False
            or checkpoint.get("new_native_starts") != 0
            or checkpoint.get("new_model_turns") != 0):
        raise ValueError("Checkpoint must remain publication-only with no new model allowance")
...
def known_trees(git, files):
    ...
    _, rows = checkpoint_rows(files[CHECKPOINT_PATH]["body"])
    ...
    for name, row in rows.items():
        body = files[name]["body"] if name in files else bodies[base[name][1]]
...
def classify_head(git, head, expected):
    ...
    if (git.text("rev-list", "--parents", "-n", "1", head).split() == [head, BASE]
        ...
        return "checkpoint029"
    raise ValueError("HEAD is neither the pinned base028 nor the exact checkpoint029 child")
...
def inspect(root, files, complete=False, quiet=False):
    ...
    base, expected, base_bodies = known_trees(git, files)
    head = git.text("rev-parse", "HEAD")
    disposition = classify_head(git, head, expected)
    complete = complete or disposition == "checkpoint029"
    index = git.index()
    ...
    for name, value in index.items():
        accepted = (expected.get(name),) if complete else (base.get(name), expected.get(name))
    ...
    if not quiet:
        ...
        print("Verified complete staged checkpoint." if complete else "Verified known base028/029 transitions.")
    return git, head, snapshot, disposition


def apply(root, files):
    git, head, snapshot, disposition = inspect(root, files)
    if disposition == "checkpoint029":
    ...
    if (git.index() == expected
            and all(snapshot[name] == (entry["body"], entry["mode"])
                    for name, entry in files.items())):
        inspect(root, files, complete=True, quiet=True)
    ...
    backup_parent = ordinary(root, "delivery/checkpoint029_backups", directory=True)
...
def main():
    ...
    if args.zip_path is None and args.sha256 is None and not (args.apply or args.verify):
        ...
        print("--apply installs/stages publication files. This helper never commits, pushes or starts a model.")
        return
    ...
    print("Publication-only checkpoint. No scientific approval or model allowance is changed.")
    for name, entry in sorted(files.items()):
