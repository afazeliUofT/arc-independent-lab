"""Local P3 checkpoint-016 ZIP handoff. No commit, push, fetch, or authentication.
...
BASE = "aef4f9edbf0fb6288a80dd37a69151fc195936ab"
...
def load_package(path, expected_hash):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if (set(manifest) != {"checkpoint_id", "base_commit", "files"}
                or manifest["checkpoint_id"] != "P3_CHECKPOINT_016"
...
def apply(root, files):
    ...
    if head != BASE:
        ...
        return
    backup_parent = ordinary(root, "delivery/checkpoint016_backups", directory=True)
