"""Local, deterministic checkpoint-028 ZIP handoff; no network or native launch.
...
approval, changing only ESCALATION.md and CHECKPOINT_028.json from the unsigned
...
BASE = "3f83ebf3c3dd8d7025ecbbaf4461c03452938e8e"
...
CHECKPOINT_ID = "P3_CHECKPOINT_028"
CHECKPOINT_PATH = "state/CHECKPOINT_028.json"
ESCALATION_PATH = "state/ESCALATION.md"
SCOPE_PATH = "configs/P3_FINITE_REVIEW_SCOPE_028.json"
APPROVAL_ID = "APPROVE_P3_FINITE_REVIEW_028_CORRECTION"
SCOPE_SHA256 = "69d8413411518a4a33998fc0a271049b125520d7854341cc5584fa9dea4056c6"
UNSIGNED_ESCALATION_SHA256 = "7f83446ce8dae9344db3c927f3639223ce9edf9f28e279e5268e5ac6211e4700"
...
def approval_metadata(approved=False):
    return {
        ...
        "method": "checkpoint028_handoff.py --approve-correction",
    }
...
def validate_unsigned(files):
    if not valid_hash(SCOPE_SHA256) or not valid_hash(UNSIGNED_ESCALATION_SHA256):
        raise ValueError("Checkpoint028 build pins have not been finalized")
...
def classify_head(git, head, expected):
    ...
    if (git.text("rev-list", "--parents", "-n", "1", head).split() == [head, BASE]
        ...
        return "checkpoint028"
    raise ValueError("HEAD is neither the pinned base027 nor the exact selected028 child")
...
def inspect(root, files, complete=False, unsigned_files=None, quiet=False):
    ...
    complete = complete or disposition == "checkpoint028"
    index = git.index()
    ...
    if not quiet:
        ...
        print("Verified complete staged checkpoint." if complete else "Verified known base027/028 transitions.")
...
def apply(root, files, unsigned_files=None):
    ...
    backup_parent = ordinary(root, "delivery/checkpoint028_backups", directory=True)
...
def main():
    ...
    parser.add_argument("--approve-correction", action="store_true", help=
                        "Explicitly approve scope028: cumulative ceilings 10 native starts / 7 turns; "
                        "8 starts / 5 turns already consumed, at most 2 additional starts / 2 turns")
    ...
    if args.approve_correction:
        print("Scope028: old cumulative ceiling 8 starts / 5 turns; corrected ceiling 10 starts / 7 turns.")
        print("Already consumed: 8 starts / 5 turns. Additional allowance: at most 2 starts / 2 turns.")
        print("Sol Max and host restrictions unchanged; bounded argument feedback and exact failure diagnostics. No spending.")
    for name, entry in sorted(files.items()):
