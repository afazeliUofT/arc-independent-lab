#!/usr/bin/env python3
"""Install the verified038 outer ZIP under the existing WSL lab and run its helper.
...
It uses no network itself; the unchanged source helper returns its Git receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_038_HOME'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 32 << 20, 16 << 20, 64 << 20, 128
ROOT_FILES = {'SHA256SUMS', 'RELEASE.json', 'stage_source_packet038.py',
              'launch038_home.py', 'README_038.md', 'README_038_HOME.md'}
PAPERS = {'private_papers/' + digest + '.pdf' for digest in (
...
def archive_payloads(raw):
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        ...
        require(ROOT_FILES | PAPERS <= set(names), 'Required package files missing')
        require(all(name in ROOT_FILES | PAPERS or
                    (name.startswith('public/') and PurePosixPath(name).suffix in
                     ('.md', '.json', '.jsonl', '.py')) for name in names),
...
def repository_check(lab):
    ...
    require(not git(lab, 'diff', '--name-only', '-z') and
            not git(lab, 'diff', '--cached', '--name-only', '-z'),
            'Tracked or staged edits preserved; run stopped')
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Running source staging and GitHub receipt publication.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'stage_source_packet038.py'),
                      '--lab', str(lab), '--bundle', str(bundle)], cwd=lab)
    require(result.returncode == 0, 'Source helper stopped; repeat this command after resolving its error')
    return bundle
