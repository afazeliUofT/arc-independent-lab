#!/usr/bin/env python3
"""Human-run publication only of declared checkpoint036 documents and evidence.
...
BASE = '17bd0df7a4d60f6ddfcffefc73049b1471a5ffd1'
REMOTE = 'https://github.com/afazeliUofT/arc-independent-lab.git'
META = 'delivery/P3_036_PUBLICATION'
...
def publish_pending(root, pending):
    ...
    if result.returncode:
        print('PUSH FAILED: local commit and pending publication preserved. Repeat the same 036 command; it retries publication only.', flush=True)
        return result.returncode
    receipt = {'kind':'P3_036_PUBLISHED_v1', 'commit':current,
...
def install(root, bundle, manifest):
    ...
    for name, body in payload.items():
        ...
        tmp = p.with_name(p.name + '.036-install')
...
def package(root, bundle):
    ...
    if (manifest.get('kind') != 'P3_CHECKPOINT_036_PACKAGE_v1'
...
def drive(root, bundle):
    ...
    if os.path.lexists(pending_path):
        ...
        expected_rows = rows
        if (pending.get('kind') != 'P3_036_PENDING_PUBLICATION_v1'
    ...
    if os.path.lexists(done_path):
        ...
        expected_rows = rows
        if (done.get('kind') != 'P3_036_PUBLISHED_v1' or done.get('files') != expected_rows
    ...
    pending = {'kind':'P3_036_PENDING_PUBLICATION_v1', 'parent':BASE,
...
COMMIT_MESSAGE = 'Phase 3: record second-pass primary-method PI audit and checkpoint035 verification'


def main():
    if sys.argv[1:] != ['--publish']:
        raise SystemExit('Use --publish for checkpoint036 document publication only.')
...
if __name__ == '__main__':
    try:
        ...
        print('Publication interrupted. Existing files are preserved; repeat the same 036 command.', file=sys.stderr)
