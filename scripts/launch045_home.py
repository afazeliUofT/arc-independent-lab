#!/usr/bin/env python3
"""Install the verified045 outer ZIP under the existing WSL lab and run its focused diagnostic conformance workflow.
...
It uses no network itself; the versioned workflow runs the bounded fabricated diagnostic checks once and publishes its report plus receipt.
"""
...
STEM = 'delivery/P3_CHECKPOINT_045'
MAX_ARCHIVE, MAX_FILE, MAX_TOTAL, MAX_MEMBERS = 64 << 20, 32 << 20, 128 << 20, 512
ROOT_FILES = {'SHA256SUMS', 'package_release.json', 'launch045_home.py', 'README_045.md'}
BOOT_SCRIPTS = {'scripts/checkpoint045_workflow.py', 'scripts/stage_source_packet038.py'}
...
def launch(raw, archive_path='', *, home=None, runner=None):
    ...
    print('Running focused fabricated diagnostic conformance once. Preserving all043/044 measurements and returning the full available report and receipt to GitHub.', flush=True)
    execute = subprocess.run if runner is None else runner
    result = execute([sys.executable, '-B', '-u', str(bundle / 'scripts/checkpoint045_workflow.py'),
                      '--lab', str(lab), '--bundle', str(bundle)], cwd=lab)
    require(result.returncode == 0, 'Report publication or repository checks did not complete; the same command reuses saved execution state and retries publication without rerunning a reserved diagnostic invocation')
    return bundle
