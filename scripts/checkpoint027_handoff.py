"""Local, deterministic checkpoint-027 ZIP handoff; no network or native launch.
...
variant. --approve-correction explicitly selects the corrected finite allowance
approval, changing only ESCALATION.md and CHECKPOINT_027.json from the unsigned
...
BASE = "1380057c0e136446fb9ec8cc35ffa883c2e5aa78"
...
CHECKPOINT_ID = "P3_CHECKPOINT_027"
CHECKPOINT_PATH = "state/CHECKPOINT_027.json"
ESCALATION_PATH = "state/ESCALATION.md"
SCOPE_PATH = "configs/P3_FINITE_REVIEW_SCOPE_027.json"
APPROVAL_ID = "APPROVE_P3_FINITE_REVIEW_027_CORRECTION"
SCOPE_SHA256 = "a69877f8bbeddfd4b638943d850e73a8eb2e978d9d1ce08600484d6c5a0df7a9"
UNSIGNED_ESCALATION_SHA256 = "37c6c0fbde106b7894606701d42899c5728f39696749e031d1e7e85b759028bc"
MAX_BYTES = 64 * 1027 * 1027
...
def approval_metadata(approved=False):
    return {
        ...
        "method": "checkpoint027_handoff.py --approve-correction",
    }
...
def validate_unsigned(files):
    if not valid_hash(SCOPE_SHA256) or not valid_hash(UNSIGNED_ESCALATION_SHA256):
        raise ValueError("Checkpoint027 build pins have not been finalized")
    for name in (SCOPE_PATH, ESCALATION_PATH, CHECKPOINT_PATH):
    ...
    # Reject ambiguity before generating anything. The fixed unsigned document
    # must leave the actual ANSWER section absent, including fenced lookalikes.
...
def classify_head(git, head, expected):
    ...
    if (git.text("rev-list", "--parents", "-n", "1", head).split() == [head, BASE]
        ...
        return "checkpoint027"
    raise ValueError("HEAD is neither the pinned base026 nor the exact selected027 child")
...
def inspect(root, files, complete=False, unsigned_files=None, quiet=False):
    ...
    complete = complete or disposition == "checkpoint027"
    index = git.index()
    ...
    if not quiet:
        ...
        print("Verified complete staged checkpoint." if complete else "Verified known base026/027 transitions.")
...
def apply(root, files, unsigned_files=None):
    ...
    backup_parent = ordinary(root, "delivery/checkpoint027_backups", directory=True)
...
def main():
    ...
    parser.add_argument("--approve-correction", action="store_true", help=
                        "Explicitly approve scope027: cumulative ceilings 8 native starts / 5 turns; "
                        "6 starts / 3 turns already consumed, at most 2 additional starts / 2 turns")
    ...
    if args.approve_correction:
        print("Scope027: old cumulative ceiling 6 starts / 3 turns; corrected ceiling 8 starts / 5 turns.")
        print("Already consumed: 6 starts / 3 turns. Additional allowance: at most 2 starts / 2 turns.")
        print("Sol Max and host restrictions unchanged; acknowledge one exact disabled-host notice. No spending.")
    for name, entry in sorted(files.items()):
