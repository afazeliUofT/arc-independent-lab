...
The fixed030 model operation is owned by review030_workflow, never by Git retry.
...
BASE = '4ed9c95b4200daf2dcc455d1f14c9a93757054a6'
REMOTE = 'https://github.com/afazeliUofT/arc-independent-lab.git'
META = 'delivery/P3_031_PUBLICATION'
POINTER = 'state/LATEST_REVIEW_030_RETURN.json'
...
def publish_pending(root, pending):
    ...
    if git(root, 'diff', '--cached', '--name-only').strip():
        ...
        subprocess.run(['git', '-C', str(root), 'commit', '-m',
                        'Phase 3: preserve reviewer030 execution evidence and GitHub return workflow'],
    ...
    if result.returncode:
        print('PUSH FAILED: local commit and pending publication preserved. Repeat the same031 command; it will retry publication without invoking the reviewer.', flush=True)
...
def install(root, bundle, manifest):
    ...
    for name, body in payload.items():
        ...
        tmp = p.with_name(p.name + '.031-install')
...
def failed_collection(root):
    ...
    workflow_path = 'delivery/P3_030_RETURN_WORKFLOW/WORKFLOW_REPORT.json'
    body = encoded({'kind':'P3_031_COLLECTION_FAILURE_v1',
    ...
    digest = sha(body)
    archive = 'evidence/P3_REVIEW_030_RETURN_FAILURES/' + digest
...
def main():
    if sys.argv[1:] != ['--run-and-publish']:
        raise SystemExit('Run with --run-and-publish to execute the already approved return workflow.')
    ...
    if os.path.lexists(pending_path):
        return publish_pending(root, json.loads(read(pending_path)))
    ...
    rows = install(root, bundle, manifest)
    workflow_path = root / 'scripts/review030_workflow.py'
    spec = importlib.util.spec_from_file_location('review030_workflow', workflow_path)
    workflow = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(workflow)
    try:
        outcome = workflow.run_workflow(root)
    except Exception:
        # Report destination/collector failures still get a categorical GitHub
        # return when the separately checked evidence directory is writable.
        outcome = {'collection':None, 'status':'WORKFLOW_COLLECTION_COULD_NOT_COMPLETE'}
    collection = outcome.get('collection')
    if not isinstance(collection, dict):
        collection = failed_collection(root)
    for name in collection['tracked_paths']:
        safe_name(name)
        if not name.startswith(('artifacts/P3_REVIEW_030_RETURN/', 'evidence/P3_REVIEW_030_RETURN_FAILURES/')):
            raise RuntimeError('Collector requested an unexpected publication path')
        rows[name] = {'sha256': sha(read(root / name))}
    pointer_body = encoded({'kind':'P3_REVIEW_030_LATEST_RETURN_v1',
    ...
    if os.path.lexists(pointer):
        # Refuse any uncommitted alteration to the previous pointer.
        old = read(pointer)
        if old != pointer_body and old != git(root, 'show', 'HEAD:' + POINTER):
            raise RuntimeError('Previous return pointer was edited; preserved')
    tmp = pointer.with_name(pointer.name + '.031-install')
    write_new_or_equal(tmp, pointer_body)
    os.replace(tmp, pointer)
    rows[POINTER] = {'sha256':sha(pointer_body)}
    pending = {'kind':'P3_031_PENDING_PUBLICATION_v1',
...
if __name__ == '__main__':
    try:
        ...
        print('Publication interrupted. Existing evidence is preserved; repeat the same031 command.', file=sys.stderr)
