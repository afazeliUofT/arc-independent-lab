...
def run_session(plan, run, requested, broker, *, mode, prompt, wall_seconds,
    ...
    """
    run = Path(run)
    report = {"kind": "P3_WARNING_CAPTURE_SESSION_026_v1", "mode": mode,
    ...
    try:
        require(mode == "synthetic" and expected_binding is None and type(prompt) is str and
                ...
                while b"\n" in incoming and not done:
                    ...
                    if "method" in message:
                        ...
                        if observed_method == "warning":
                            ...
                            raise Stop("Generic warning stopped diagnostic; safe classification preserved")
